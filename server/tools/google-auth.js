...
async function connectGoogle(settings, emit) {
  ...
  if (!cfg.google_client_id || !cfg.google_client_secret) {
    throw new Error('Google Client ID y Client Secret son requeridos.')
  }
  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer(async (req, res) => {
      ...
      if (!code) { res.end('Error: no code'); server.close(); reject(new Error('No code')); return }
      try {
        const { tokens } = await oauth2Client.getToken(code)
        settings.save({ google_authenticated: true, google_tokens: tokens })
        emit({ type: 'google_connected' })
        res.end('<html><body><h2>✓ Morpheus conectado a Google. Cierra esta ventana.</h2></body></html>')
        server.close()
        resolve(tokens)
      } catch (err) { res.end('Error'); server.close(); reject(err) }
    })
    server.listen(PORT, () => {
      // open the browser
